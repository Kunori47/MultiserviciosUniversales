...
const TableSpecialty = ({ specialties, onDelete }: Props) => {
  ...
  // Filtrar especialidades por descripción
  const filteredSpecialties = specialties.filter((specialty) =>
    specialty.DescripcionEspecialidad.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const numPages = Math.ceil(filteredSpecialties.length / perPage);
  const pagesList: number[] = [];
  for (let i = 0; i < numPages; i++) pagesList.push(i);

  const specialtiesPaginated = filteredSpecialties.slice(
    perPage * currentPage,
